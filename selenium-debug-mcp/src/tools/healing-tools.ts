...
let testWatcher: TestWatcher | null = null;

export function registerHealingTools(frameworkRoot: string) {
  ...
  return [
...

    {
      ...
      handler: async (args: any) => {
        const result = await codeModifier.updateSelectorJson(
          args.selectorFile,
          args.selectorName,
          { type: args.newType, value: args.newValue }
        );
...

    {
      ...
      handler: async (args: any) => {
        const result = await codeModifier.updatePageObjectSelector(
          args.pageClassName,
          args.selectorVariable,
          args.newType,
          args.newValue
        );
...

    {
      ...
      handler: async (args: any) => {
        const result = await codeModifier.updateWaitTime(
          args.javaFile,
          args.currentSeconds,
          args.newSeconds
        );
