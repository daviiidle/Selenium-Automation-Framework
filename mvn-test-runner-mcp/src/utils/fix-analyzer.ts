...
export class FixAnalyzer {
   ...
   */
  private analyzeFailure(failure: TestFailure): FixSuggestion {
   ...
   */
  private analyzeSelectorFailure(failure: TestFailure): FixSuggestion {
...
- update-selector-file: Update JSON selector file
   ...
   */
  private analyzeTimeoutFailure(failure: TestFailure): FixSuggestion {
...
- update-wait-time: Update wait timeout in code
   ...
   */
  private analyzeAssertionFailure(failure: TestFailure): FixSuggestion {
...
- Application has regression
   ...
   */
  private analyzeConfigurationFailure(failure: TestFailure): FixSuggestion {
    return {
      testClass: failure.testClass,
      testMethod: failure.testMethod,
...
- check-testng-config: Validate TestNG XML
   ...
   */
  private analyzeUnknownFailure(failure: TestFailure): FixSuggestion {
    return {
      testClass: failure.testClass,
      testMethod: failure.testMethod,
...
4. Review recent code changes
  ...
  groupFailuresByCategory(failures: TestFailure[]): Map<string, TestFailure[]> {
    const grouped = new Map<string, TestFailure[]>();
