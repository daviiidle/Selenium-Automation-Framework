...
export function registerMavenTestTools(frameworkRoot) {
    ...
    return [
        ...
        },
        {
            ...
            handler: async () => {
                ...
                return {
                    summary: parsedResults.summary,
                    failuresByCategory: Object.fromEntries(Array.from(groupedFailures.entries()).map(([cat, failures]) => [
                        cat,
                        failures.map(f => ({
        ...
        },
        {
            ...
            handler: async (args) => {
                ...
                const parsedResults = await testParser.parseTestResults(result.stdout + result.stderr);
                const failure = parsedResults.failures.find(f => f.testClass === testClass && f.testMethod === testMethod);
                if (!failure) {
                    return {
                        success: true,
        ...
        },
        {
            ...
            handler: async () => {
                ...
                return {
                    ...
                    patterns,
                    categoryBreakdown: Object.fromEntries(Array.from(groupedFailures.entries()).map(([cat, failures]) => [
                        cat,
                        failures.length,
                    ])),
    ...
    function getRelatedTools(category) {
        const toolMap = {
            SELECTOR: [
    ...
    function getPatternRecommendations(patterns, groupedFailures) {
        const recommendations = [];
        const selectorCount = groupedFailures.get('SELECTOR')?.length || 0;
        const timeoutCount = groupedFailures.get('TIMEOUT')?.length || 0;
        if (selectorCount > 5) {
            recommendations.push('Multiple selector failures detected - consider reviewing page structure changes or updating all selectors at once');
        }
        if (timeoutCount > 3) {
            recommendations.push('Multiple timeout failures - consider increasing global wait times or checking application performance');
        }
        if (patterns.some(p => p.includes('NoSuchElementException'))) {
            recommendations.push('NoSuchElementException is common - use validate-selector-json to check all selectors on live site');
        }
        return recommendations;
    }
}
//# sourceMappingURL=maven-test-tools.js.map